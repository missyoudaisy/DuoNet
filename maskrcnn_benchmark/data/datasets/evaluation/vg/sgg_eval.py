...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode, output_folder=None, img_id=None):
        ...
        if mode == 'predcls' or mode == 'sgcls':
            ...
            save_dict['sort_idx'] = sort_idx.tolist()
        ...
        save_dict['gt_triplets'] = gt_triplets.tolist()

        pred_triplets, pred_triplet_boxes, pred_triplet_scores = _triplet(
                pred_rels, pred_classes, pred_boxes, pred_scores, obj_scores)
        save_dict['pred_triplets'] = pred_triplets.tolist()
        save_dict['pred_triplet_scores'] = pred_triplet_scores.tolist()

        # Compute recall. It's most efficient to match once and then do recall after
        pred_to_gt, pred_match_rel_id = _compute_pred_matches(
            gt_triplets,
            pred_triplets,
            gt_triplet_boxes,
            pred_triplet_boxes,
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        nogc_pred_to_gt, _ = _compute_pred_matches(
            gt_triplets,
            nogc_pred_triplets,
            gt_triplet_boxes,
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    pred_match_rel_id = [[] for x in range(pred_boxes.shape[0])]
    for gt_ind, gt_box, keep_inds in zip(np.where(gt_has_match)[0],
        ...
        for i in np.where(keep_inds)[0][inds]:
            ...
            pred_match_rel_id[i].append(int(gt_triplets[gt_ind][1]))
    return pred_to_gt, pred_match_rel_id
