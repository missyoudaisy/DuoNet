...
def do_vg_evaluation(
    ...
    if cfg.GLOBAL_SETTING.DATASET_CHOICE == 'VG':
        ...
        num_rel_category = cfg.MODEL.ROI_RELATION_HEAD.OIV6_NUM_CLASSES
    ...
    if "relations" in iou_types:
        ...
        for img_id, (groundtruth, prediction) in enumerate(zip(groundtruths, predictions)):    
            evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, output_folder, img_id)
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, output_folder, img_id):
    """
    Returns:
    ...
    local_container = evaluator['eval_recall'].calculate_recall(global_container, local_container, mode, output_folder, img_id)
