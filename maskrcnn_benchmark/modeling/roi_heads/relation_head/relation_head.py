...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, iteration=None, image_ids=None, logger=None):
        """
        Arguments:
        ...
        if self.training:
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    proposals, rel_labels, rel_pair_idxs, rel_binarys, target_labels = self.samp_processor.gtbox_relsample(proposals, targets)
                else:
                    proposals, rel_labels, rel_pair_idxs, rel_binarys, target_labels = self.samp_processor.detect_relsample(proposals, targets)
        else:
        ...
        refine_logits, relation_logits, add_losses, add_data = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, target_labels, iteration, logger)
