...
def compute_on_dataset(model, data_loader, device, output_folder, synchronize_gather=True, timer=None):
    model.eval()
    results_dict = {}
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                iteration = -1
                output = model(images.to(device), targets, iteration)
            if timer:
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        predictions = compute_on_dataset(model, data_loader, device, output_folder, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer)
